...
/// "Choose Exam" — mock-exam variant of the report list picker. Tapping
/// the View button navigates to `Routes.masterReport` (with predictive
/// flag) when access is granted; otherwise the no-access gate appears.
///
/// Preserved public contract:
///   • Constructor `SelectMasterExamReportList({super.key,
///     required this.id, required this.type,
///     required this.showPredictive})`
///   • Static `route(RouteSettings)` reads `id` / `type` /
///     `showPredictive`.
///   • `store.onMasterReportExamByCategoryApiCall(widget.id)` fired in
///     initState.
///   • Lowercase-contains search on `examName`.
///   • `isAccess == true` → `Routes.masterReport` with
///     `{id, title, showPredictive: widget.showPredictive}`.
///   • `isAccess == false` → Windows: `AlertDialog(NoAccessAlertDialog)`;
///     else modal bottom sheet `NoAccessBottomSheet`.
///   • Empty state copy preserved verbatim.
///   • `!store.isConnected` → `NoInternetScreen`.
class SelectMasterExamReportList extends StatefulWidget {
  final String id;
  final String type;
  final bool showPredictive;
  const SelectMasterExamReportList(
      {super.key,
      required this.id,
      required this.type,
      required this.showPredictive});
  ...
  @override
  State<SelectMasterExamReportList> createState() =>
      _SelectMasterExamReportListState();

  static Route<dynamic> route(RouteSettings routeSettings) {
    final arguments = routeSettings.arguments as Map<String, dynamic>;
    return CupertinoPageRoute(
      builder: (_) => SelectMasterExamReportList(
        id: arguments['id'],
        type: arguments['type'],
...
class _SelectMasterExamReportListState
    extends State<SelectMasterExamReportList> {
  ...
  void initState() {
    ...
    store.onMasterReportExamByCategoryApiCall(widget.id);
  }
          ...
          Expanded(
            child: Observer(
              builder: (_) {
                ...
                if (store.masterReportByExam.isEmpty) {
                  return Padding(
                ...
                return ListView.separated(
                  itemCount: store.masterReportByExam.length,
                  shrinkWrap: true,
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    ReportByExamListModel? reportByExam =
                        store.masterReportByExam[index];
                    if (query.isNotEmpty &&
