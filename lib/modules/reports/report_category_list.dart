...
/// "Analysis & Solutions" — regular report category list.
///
/// Preserved public contract:
///   • Constructor `ReportCategoryScreen({super.key, this.fromHome})`
///   • Static `route(RouteSettings)` reads `fromhome` (lowercase).
///   • `WillPopScope` pushes `Routes.dashboard` with
///     `{'fromhome': widget.fromHome}`.
///   • Back button pushes `Routes.dashboard`.
///   • `store.onReportCategoryApiCall(context)` in initState.
///   • `SubscriptionStore.onGetSubscribedUserPlan()` in initState.
///   • Lowercase-contains search on `category_name`.
///   • Empty state copy verbatim + "Start a Test" → `Routes.testCategory`.
///   • Tap → `Routes.reportsSubCategoryList` with
///     `{categoryName, categoryId}`.
///   • `!store.isConnected` → `NoInternetScreen`.
class ReportCategoryScreen extends StatefulWidget {
  final bool? fromHome;
  const ReportCategoryScreen({super.key, this.fromHome});
  ...
  static Route<dynamic> route(RouteSettings routeSettings) {
    ...
    return CupertinoPageRoute(
      builder: (_) => ReportCategoryScreen(fromHome: arguments['fromhome']),
    );
  }
}
...
class _ReportCategoryScreenState extends State<ReportCategoryScreen> {
  ...
  void initState() {
    ...
    store.onReportCategoryApiCall(context);
  }
    ...
    return WillPopScope(
      onWillPop: () async {
        Navigator.of(context).pushNamed(
          Routes.dashboard,
          arguments: {'fromhome': widget.fromHome},
        );
        return false;
      },
      child: Scaffold(
              ...
              child: Observer(
                builder: (_) {
                  ...
                  return ListView.separated(
                    itemCount: store.bookmarkCategory.length,
                    shrinkWrap: true,
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      ...
                      return InkWell(
                        ...
                        onTap: () {
                          Navigator.of(context).pushNamed(
                            Routes.reportsSubCategoryList,
                            arguments: {
                              "categoryName": bookMarkCat?.category_name,
                              "categoryId": bookMarkCat?.category_id,
                            },
                          );
                        },
                        child: Container(
