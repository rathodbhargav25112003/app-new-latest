...
/// NoAccessBottomSheet — paywall / free-trial prompt bottom sheet
/// (mobile-first counterpart of [NoAccessAlertDialog]). Public surface
/// preserved exactly:
...
///     [FreeTrialSuccessBottomSheet]; upgrade branch pushes
///     [Routes.newSubscription] with `{'showBackButton': true}`
class NoAccessBottomSheet extends StatefulWidget {
  final bool isFree;
  final String planId;
  final int day;
  final Function()? onTap;

  const NoAccessBottomSheet({
    super.key,
    this.isFree = false,
    this.planId = "",
    this.day = 0,
  ...
  @override
  State<NoAccessBottomSheet> createState() => _NoAccessBottomSheetState();
}

class _NoAccessBottomSheetState extends State<NoAccessBottomSheet> {
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: AppTokens.s20),
            _GradientCta(
              label: !widget.isFree ? 'Activate Free Trial' : 'View Plans',
              icon: !widget.isFree
