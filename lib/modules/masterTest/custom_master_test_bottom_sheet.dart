...
/// Master test instructions bottom sheet — redesigned with AppTokens.
/// Preserves the positional-arg constructor, the internal
/// CarouselSliderControllerImpl field, the isPractice-driven height
/// (55% vs 30% of screen), platform rounded-border decoration on
/// desktop, the full 7-legend instruction dots, the
/// +marksAwarded/-marksDeducted marking-scheme row (with the original
/// svg assets), ExpandableText for the raw instruction, and the
/// entire `_startMasterExamApiCall` chain — onCreateTestHistoryCall,
/// onGetMaterExamPaperDataApiCall (with identical TestData/Options
/// mapping), startCreateMaterExam, ExamScreen route for MockExam,
/// Routes.practiceMasterTestExams for practice, and BottomToast error
/// handling.
class CustomMasterTestBottomSheet extends StatefulWidget {
  final TestExamPaperListModel? testExamPaper;
  final String? id;
  final String? type;
  final bool? isPractice;
  final bool? showPredictive;
  const CustomMasterTestBottomSheet(BuildContext context, this.testExamPaper,
      this.id, this.type, this.isPractice, this.showPredictive,
      {super.key});

  @override
  State<CustomMasterTestBottomSheet> createState() =>
      _CustomMasterTestBottomSheetState();
}

class _CustomMasterTestBottomSheetState
    extends State<CustomMasterTestBottomSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    final double height = isPractice
    ...
    return SingleChildScrollView(
      child: Container(
        ...
        child: Column(
          ...
          children: [
            if (!isDesktop) const SizedBox(height: AppTokens.s12),
            if (!isDesktop)
              Container(
                width: 44,
                height: 4,
              ...
              child: Observer(
                builder: (_) {
                  return Column(
                    ...
                    children: [
                      _SheetHeader(examName: widget.testExamPaper?.examName ?? ""),
                      if (!isPractice) ...[
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                if ((widget.testExamPaper?.instruction ?? "")
                                    ...
                                    .isNotEmpty) ...[
                                  const SizedBox(height: AppTokens.s16),
                                  _SectionLabel(text: "Additional Notes"),
