...
class _VerificationOtpState extends State<VerificationOtp>
                ...
                const SizedBox(height: AppTokens.s24),
                    ...
                    OtpAutofillField(
                      ...
                      onCompleted: (code) {
                        ...
                        _verifyOtpPhone(
                            store, loginStore, widget.email, otp);
                      },
                    ),
                    const SizedBox(height: AppTokens.s24),
                          ...
                          if (!_timerActive)
                            InkWell(
                              onTap: () {
                                _startTimer();
                                _resendOtpPhone(
                                    store, widget.email, widget.email2);
                              },
                              child: Text(
                                "Resend OTP",
  ...
  Future<void> _resendOtp(SignupStore store, String email) async {
    await store
        .onSendOtpToMail(email, widget.registerData.fullName)
        .then((value) {
  ...
  Future<void> _resendOtpPhone(
      SignupStore store, String phone, String email) async {
    await store.onSendOtpToPhone(phone, email).then((value) {
      if (store.errorMessageOtp2.value?.message != null) {
        ...
        BottomToast.showBottomToastOverlay(
          context: context,
          errorMessage:
              store.errorMessageOtp2.value?.error ?? "Something went wrong!",
          backgroundColor: Theme.of(context).colorScheme.error,
        );
      }
    });
  ...
  Future<void> _verifyOtpMail(SignupStore store, LoginStore loginStore,
    ...
    await store.onVerifyOtpToMail(otp, email).then((value) async {
      if (store.registerWithEmail.value?.message != null) {
        await store.onRegisterWithPhoneApiCall(
            fullName,
            dateOfBirth,
            preparingVal,
            stateVal,
            ...
            phoneNo,
            email,
            isTablet(context) ? "tab" : "",
            standardId: widget.registerData.standardId,
            preparingId: widget.registerData.preparingId);
        ...
        if (store.signupWithPhone.value?.created == null) {
          ...
          if (store.signupWithPhone.value?.data?.token != null) {
            ...
            prefs.setString(
                'token', store.signupWithPhone.value?.data?.token ?? "");
            ...
            Navigator.of(context)
                .pushNamed(Routes.home, arguments: {'trial': widget.trial});
          }
        }
        ...
        BottomToast.showBottomToastOverlay(
          context: context,
          errorMessage:
              store.registerWithEmail.value?.error ?? "Something went wrong!",
          backgroundColor: Theme.of(context).colorScheme.error,
        );
      }
    });
  ...
  Future<void> _verifyOtpPhone(SignupStore store, LoginStore loginStore,
    ...
    await store.onVerifyOtpToPhone(phoneNo, otp).then((value) async {
      if (store.registerWithEmail2.value?.message != null) {
        ...
        BottomToast.showBottomToastOverlay(
          context: context,
          errorMessage:
              store.registerWithEmail2.value?.error ?? "Something went wrong!",
          backgroundColor: Theme.of(context).colorScheme.error,
        );
      }
    });
