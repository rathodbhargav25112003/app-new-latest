...
/// "Offline Notes" subcategory-list screen. Reads previously-downloaded
/// notes (grouped by `subCategoryId`) for a given `categoryId` from
/// the local Hive store via
/// `DbHelper.getAllNotesGroupedBySubCategoryId(categoryId)`, then
/// pushes into `Routes.downloadedNotesTopic` on tile tap.
///
/// Preserved public contract:
///   • `OfflineSubCategoryList({super.key, this.categoryId})` with
///     nullable `String? categoryId`.
///   • Static `route(RouteSettings)` factory reading
///     `arguments['categoryId']` and returning `CupertinoPageRoute`.
///   • Navigator push `Routes.downloadedNotesTopic` with argument map
///     `{ 'subcategoryId': notesCat?.subCategoryId }` preserved.
///   • `NotesCategoryStore.onSearchApiCall(keyword, "PDF")` preserved.
class OfflineSubCategoryList extends StatefulWidget {
  const OfflineSubCategoryList({super.key, this.categoryId});

  final String? categoryId;

  @override
  State<OfflineSubCategoryList> createState() =>
      _OfflineSubCategoryListState();

  static Route<dynamic> route(RouteSettings routeSettings) {
    final arguments = routeSettings.arguments as Map<String, dynamic>;
    return CupertinoPageRoute(
      builder: (_) => OfflineSubCategoryList(
...
class _OfflineSubCategoryListState extends State<OfflineSubCategoryList> {
  String filterValue = '';
  String query = '';
  ...
  Future<void> _getOfflineData() async {
    ...
    notesList = await dbHelper
        .getAllNotesGroupedBySubCategoryId(widget.categoryId ?? "");
    setState(() {
      isLoading = false;
    });
  }
