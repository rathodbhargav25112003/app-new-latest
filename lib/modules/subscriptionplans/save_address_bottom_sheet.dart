...
/// SaveAddressBottomSheet — bottom sheet used from the address list to add
/// a new shipping address. On submit it calls
/// [SubscriptionStore.onCreateAddress].
///
/// Public surface preserved exactly:
///   • class [SaveAddressBottomSheet] + const constructor `{super.key}`
///   • private [addAddressData] signature (buildingNumber, landMark,
...
///   • [_emailRegex] pattern
///   • All hint labels / CTA copy ("Save Address")
class SaveAddressBottomSheet extends StatefulWidget {
  const SaveAddressBottomSheet({super.key});
...
class _SaveAddressBottomSheetState extends State<SaveAddressBottomSheet> {
  ...
  Future<void> addAddressData(
    ...
    await store.onCreateAddress(context, buildingNumber, landMark, pinCode,
        city, state, phone, name, email);
