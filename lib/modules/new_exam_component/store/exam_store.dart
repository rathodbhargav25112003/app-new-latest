...
abstract class _ExamStore extends InternetStore with Store {
  ...
  static const int triggerInterval = 1 * 60; // 2 minutes in seconds
  ...
  /// Call from the widget's dispose() / deactivate() when leaving the exam
  /// screen. Prevents a leaked periodic timer from continuing to call
  /// _triggerAction() on an unmounted widget (which previously only
  /// happened if resetFlags was reached, i.e. only on submit).
  void stopTimer() {
    _timer?.cancel();
    _timer = null;
  }
